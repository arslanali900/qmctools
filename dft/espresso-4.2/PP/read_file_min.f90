...
SUBROUTINE read_file_min()
  ...
  !
  !JNKIM CALL init_us_1()
  !JNKIM IF (okpaw) then
  !JNKIM    becsum = rho%bec
  !JNKIM    CALL PAW_potential(rho%bec, ddd_PAW)
  !JNKIM ENDIF 
  !JNKIM if ( real_space ) THEN !initialisation of real space related stuff
  !JNKIM   !OBM - correct parellism issues
  !JNKIM   !call qpointlist()
  !JNKIM   call betapointlist()
  !JNKIM   call init_realspace_vars()
  !JNKIM   !call betapointlist_v2()
  !JNKIM   write(stdout,'(5X,"Real space initialisation completed")')
  !JNKIM endif
  !JNKIM CALL newd()
