...
void TestShiftedChebyshev()
{
  int N = 6;
  ShiftedChebyshevPoly<double> cheb(N);

  FILE *fout   = fopen ("ShiftedChebyshev.dat", "w");
  FILE *dfout  = fopen ("dShiftedChebyshev.dat", "w");
  FILE *d2fout = fopen ("d2ShiftedChebyshev.dat", "w");
...
TestFit ()
{
  double k = 8.1943294112345;
  int nquad = 31;
  ShiftedChebyshevQuad rule(nquad);
  int N = 31;
  ShiftedChebyshevPoly<double> cheb(N);
